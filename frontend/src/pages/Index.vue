<template>
  <div class="container col-md-7 col-sm-12 sm-container">
      ...
      <div v-else-if="groups.length === 0 && services.length === 0 && messages === null" class="row mt-5 mb-5">
        <div class="col-12 text-center mt-3 mb-3">
          <span class="text-dim">Aucun groupe, service ou message à afficher.</span>
        </div>
      </div>
...
export default {
  ...
  data() {
      return {
          ...
          messages: null // Initialiser messages à null
      };
  },
  computed: {
      loadingMessage() {
          ...
          return ""; // Pour éviter une erreur si aucun n'est en chargement
      },
      groups() {
  ...
  async mounted() {
      ...
      try {
          await this.$store.dispatch('loadMessages'); // Dispatch pour charger les messages
